...
var conf = fs.readFileSync(process.cwd() + '/config/passport.yml', 'utf8').toString();
...
exports.init = function (compound) {
    ...
    app.get((app.settings.passportRoot || '') + '/auth/:idp/*', function (req, res) {
        res.send('Provider `' + req.params.idp + '` is not enabled. Specify appropriated settings in config/passport.yml file');
    });
    
    app.get((app.settings.passportRoot || '') + '/auth/logout', function(req, res){
      req.logout();
      req.session.destroy();
